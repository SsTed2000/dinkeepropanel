...
if not game:IsLoaded() then
	...
	notLoaded.Text = 'u cant be pro if gam not launched'
	game.Loaded:Wait()
	notLoaded:Destroy()
end
...
Title.TextSize = 18
Title.Text = "Dinkee Pro Panel v"..ver
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- Dinkee Pro Panel Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
CMDs = {}
CMDs[#CMDs + 1] = {NAME = 'discord / support / help', DESC = 'Invite to the Aquataria server.'}
...
addcmd('discord', {'support', 'help'}, function(args, speaker)
	...
	if toClipboard then
		toClipboard('https://discord.gg/FUrddEg')
		notify('Discord Invite', 'Copied to clipboard!\ndiscord.gg/FUrddEg')
	else
		notify('Discord Invite', 'discord.gg/FUrddEg')
	end
...
task.spawn(function()
	if pcall(function() loadstring(game:HttpGet('https://raw.githubusercontent.com/SsTed2000/dinkeepropanel/main/version'))() end) then
		if ver ~= Version then
			notify('Outdated!','Bro, you CANT be pro if its not the latest version!')
		end
